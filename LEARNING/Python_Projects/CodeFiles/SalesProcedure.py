...
for count1 in Company:
    ...
    Query = "select "+str(count1)+''' as company, t$cofc as dpst, to_char(t$tran) as trantype, t$orno as orderno, t$pono as positionno,
    ...
    sum(t$dqua) as deliveryqty, sum(t$amth$1) as sales_amount,to_char(t$sotp) as saleordertype,t$stoa as stoa,t$cvat as taxcode
    from erpnln.tcisli310''' + str(count1) + \
    " group by t$cofc, to_char(t$tran), t$orno, t$pono, t$idoc, t$ofbp, to_char(t$item), t$rate$1, t$rate$2, t$txai,to_char(t$sotp),t$stoa,t$cvat"
...
#DattypeConversion
LN_Sales["trantype"]=LN_Sales["trantype"].astype(str) #working : Datattype conversion of a column in dataframe
LN_Sales["saleordertype"]=LN_Sales["saleordertype"].astype(str) #working : Datattype conversion of a column in dataframe
LN_Sales["item"]=LN_Sales["item"].astype(str) #working : Datattype conversion of a column in dataframe
LN_Sales["rate1"]=LN_Sales["rate1"].astype(float)
LN_Sales["rate2"]=LN_Sales["rate2"].astype(float)
LN_Sales["ed_amount"]=LN_Sales["ed_amount"].astype(float)
LN_Sales["amth1"]=LN_Sales["amth1"].astype(float)
LN_Sales["txah1"]=LN_Sales["txah1"].astype(float)
LN_Sales["foreign_currency_amount"]=LN_Sales["foreign_currency_amount"].astype(float)
LN_Sales["unitprice"]=LN_Sales["unitprice"].astype(float)
LN_Sales["deliveryqty"]=LN_Sales["deliveryqty"].astype(float)
LN_Sales["sales_amount"]=LN_Sales["sales_amount"].astype(float)
