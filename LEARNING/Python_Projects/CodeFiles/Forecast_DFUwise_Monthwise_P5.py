...
# Region=['Channel','Direct','C&M','WW','Railway Units','Railways Wiper','HO-AFM-Industrial',
# 'ACP-AFM-Industrial','HO-AFM-Portables','ACP-AFM-Portables',
# 'HO-AFM-Railway','ACP-AFM-Railway'
# #'GSC-AFM-Portables','GSC-AFM-Railway','GSC-AFM-Industrial'
# ]
#Region=['South Asia','Africa','ID','ME','Australia','Indonesia','Malaysia']
#Region=['GSC-AFM-Industrial']
#print(Organ)
...
df_final = pd.DataFrame(columns=['Region','DFUCode','DPSTCode','India_tpl','MonthEndDate','Index_date','Qty',\
                                 'DeliveryQty','ProdQty','Index_date2','y_pred'])
    ...
    for j in DFUCode:
        ...
        for k in DPSTCode:
            ...
            DateMapping = {'Index_date': ['2025-08-01', '2025-08-02', '2025-08-03','2025-08-04','2025-08-05',
                            ...
                            'Monthend_date': ['2023-11-30','2023-12-31','2024-01-31',
            ...
            #print(df1_stage)
            df1_new=pd.DataFrame(columns=['Region','DFUCode','DPSTCode','India_tpl','MonthEndDate','Qty','DeliveryQty','ProdQty'])
            df1_new[['Region','DFUCode','DPSTCode','India_tpl','Index_date','MonthEndDate','Qty','DeliveryQty','ProdQty']]=df1_stage[['Region','DFUCode','DPSTCode','India_tpl','Index_date','MonthEndDate','Qty','DeliveryQty','ProdQty']].reset_index(drop=True)    
            #print(df1_new)
            # print (df1_new.dtypes)
            # input("")
            ...
            df1_new['DeliveryQty'] = df1_new['DeliveryQty'].fillna(0)
            df1_new['ProdQty'] = 0
            
            # df1_new['DeliveryQty'] = 0
            # df1_new['ProdQty'] = df1_new['ProdQty'].fillna(0)
            ...
            #Commented on 28-11-2025
            #series=df1_new['Qty'].astype(float)
            #Added on 28-11-2025
            series=df1_new['DeliveryQty'].astype(float)
            #series=df1_new['ProdQty'].astype(float)
            ...
            df_combined = pd.concat([df1_new[['Region','DFUCode','DPSTCode','India_tpl','MonthEndDate','Index_date','Qty','DeliveryQty','ProdQty']], pred_model2[['Index_date2','y_pred',"best_model_name","elapsed_mins"]]], ignore_index=True)
            #print("df_combined :\n",df_combined)
            ...
            #Added on 28-11-2025
            df_combined.loc[pd.isna(df_combined['DeliveryQty']), 'DeliveryQty'] = df_combined.loc[pd.notna(df_combined['y_pred']), 'y_pred']
            #df_combined.loc[pd.isna(df_combined['ProdQty']), 'ProdQty'] = df_combined.loc[pd.notna(df_combined['y_pred']), 'y_pred']
            ...
            df_combined['DeliveryQty'] = df_combined['DeliveryQty'].replace('', 0)
            df_combined['ProdQty'] = df_combined['ProdQty'].replace('', 0)
            df_combined['Qty'] = df_combined['Qty'].replace('', 0)
            df_combined['elapsed_mins'] = df_combined['elapsed_mins'].replace('', 0)
            ...
            # print(df_combined.dtypes)
            # input()
            df_combined['DeliveryQty'] = df_combined['DeliveryQty'].astype(np.float16)
            df_combined['ProdQty'] = df_combined['ProdQty'].astype(np.float16)
            ...
            #Added on 28-11-2025
            df_final['DeliveryQty'] = df_final['DeliveryQty'].replace([np.inf, -np.inf], np.nan).fillna(0)
            df_final['DeliveryQty'] = df_final['DeliveryQty'].astype(np.float64)

            df_final['ProdQty'] = df_final['ProdQty'].replace([np.inf, -np.inf], np.nan).fillna(0)
            df_final['ProdQty'] = df_final['ProdQty'].astype(np.float64)
            ...
            insert_sql = "INSERT INTO dbo.SalesQtyForecast (Organ,DFUCode,DPSTCode,India_tpl,MonthendDate,IndexDate,Qty,DeliveryQty,ProdQty,ModelName,Elapsed_mins)VALUES (?,?,?,?,?,?,?,?,?,?,?)"
            ...
            df_final2=df_final[['Region','DFUCode','DPSTCode','India_tpl','MonthEndDate','Index_date','Qty','DeliveryQty','ProdQty',"best_model_name","elapsed_mins"]]
